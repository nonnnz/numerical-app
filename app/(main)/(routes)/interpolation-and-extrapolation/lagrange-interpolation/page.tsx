...
// import LagrangeInterpolationPlot from "@/app/(main)/_components/lagrangeplot";
...
// Dynamically import client-side only components (e.g., LagrangeInterpolationPlot)
const LagrangeInterpolationPlot = dynamic(
  () => import("@/app/(main)/_components/lagrangeplot"),
  {
...
const LagrangeInterpolation = () => {
  ...
  const lagrangeInterpolation = () => {
    ...
    for (let i = 0; i < n; i++) {
      ...
      for (let j = 0; j < n; j++) {
        if (i !== j) {
          term *= (x[j] - pfind) / (x[j] - x[i]);
        }
      }
      result += term * y[i];
      console.log(term, y[i]);
    }
      ...
      <Card className="w-full md:w-[705px] xl:w-[1214px]">
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          <div className="space-y-2">
            ...
            <CardDescription>
              find the value of f({find}) using Lagrange Interpolation
            </CardDescription>
          </div>
          <Popover>
        ...
        <CardFooter className="flex justify-between">
          ...
          <div className="flex items-center">
            ...
            <Button onClick={lagrangeInterpolation}>Calculate</Button>
          </div>
        </CardFooter>
      </Card>
      ...
      {result !== null && (
        <>
          ...
          <div className="rounded-md border bg-card w-full md:w-[705px] xl:w-[1214px]">
            <LagrangeInterpolationPlot
              x={x_in.map((value) => parseFloat(value))}
              y={y_in.map((value) => parseFloat(value))}
              find={parseFloat(find)}
            />
          </div>
        </>
      )}
