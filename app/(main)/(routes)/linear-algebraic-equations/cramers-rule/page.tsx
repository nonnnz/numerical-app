...
const CramersRule = () => {
    ...
    const cramersRule = () => {
        ...
        if (math.abs(detA) < 1e-10) {
            alert("Determinant is close to zero;");
            return;
        }

        const x: number[] = [];
        
        solution.push({matrixA: A, det: detA});

        for (let i = 0; i < n; i++) {
            for (let j = 0; j < n; j++) {
              A[j][i] = vectorB[j];
            }
            const detMatrix = math.clone(A);
            const det = math.det(detMatrix);
            const x_i = det / detA;
            x.push(x_i);
            solution.push({matrixA: detMatrix, det: det});
            array.push({iter: i+1, det: det, detA: detA, x: x_i});
            for (let j = 0; j < n; j++) {
              A[j][i] = matrixA[j][i];
            }
        }
        ...
        <div className="h-full flex-1 flex-col  items-center space-y-4 p-8 md:flex">
            <Card className="w-full md:w-[705px] xl:w-[1214px]">
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                    <div className='space-y-2'>
                        <CardTitle>Cramer's Rule</CardTitle>
                        <CardDescription>
                            Find the solution of a system of linear equations using Cramer's Rule
                        </CardDescription>
                    </div>
                    <Popover>
                ...
                <CardFooter className="flex justify-between">
                    ...
                    <div className="flex items-center">
                        ...
                        <Button onClick={cramersRule}>Calculate</Button>
                    </div>
                </CardFooter>
            </Card>
            ...
            {result.length > 0 ? (
                <>
                    ...
                    <Card className='w-full md:w-[705px] xl:w-[1214px]'>
                        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                            <CardTitle>Solution by Cramer's Rule</CardTitle>
                        </CardHeader>
                        <CardContent>
                            {sol.map((row, i) => (
                                <div key={i} className="flex flex-col p-4">
                                    <div className='flex flex-row gap-x-4 items-center'>
                                        <div className="">{`Δ${i === 0 ? '':i}`} = </div>
                                        <Matrix matrix={row.matrixA} variant="bracket" />
                                        <div>= {row.det}</div>
                                    </div>
                                </div>
                            ))}
                            {array.map((row, i) => (
                                <div key={i} className="flex flex-row gap-4">
                                        ...
                                        <div>{`Δ${i+1}`}/Δ = </div>
                                        <div className="">{`${removeZero(row.det)} / ${removeZero(row.detA)} = ${removeZero(row.x)}`}</div>
                                </div>
                            ))}
                        </CardContent>
                    </Card>
