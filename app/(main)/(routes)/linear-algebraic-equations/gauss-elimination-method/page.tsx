...
const GaussElimination = () => {
    ...
    const gaussEli = () => {
        ...
        let solution: any[] = [];
        let array: any[] = [];
        const max_zero = Math.pow(10, -14);
        solution.push({matrixA: math.clone(A), matrixB: math.clone(vectorB)});
        // Forward Elimination
        for (let k = 0; k < n - 1; k++) {
            for (let i = k + 1; i < n; i++) {
                ...
                vectorB[i] = math.abs(vectorB[i]) < max_zero ? 0 : vectorB[i];
                solution.push({matrixA: math.clone(A), matrixB: math.clone(vectorB)});
            }
        }
        console.log(math.clone(A));

        // Back Substitution
        let x: number[] = Array(n).fill(0);
        x[n - 1] = vectorB[n - 1] / A[n - 1][n - 1];
        for (let i = n - 1; i >= 0; i--) {
            let sum = vectorB[i];
            for (let j = i + 1; j < n; j++) {
                sum -= A[i][j] * x[j];
            ...
            x[i] = sum / A[i][i];
            array.push({sum: sum, pivot: A[i][i], x: x[i]});
        }
        ...
        <div className="h-full flex-1 flex-col  items-center space-y-4 p-8 md:flex">
            <Card className="w-full md:w-[705px] xl:w-[1214px]">
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                    <div className='space-y-2'>
                        <CardTitle>Gauss Elimination Method</CardTitle>
                        <CardDescription>
                            Find the solution of a system of linear equations using Gauss Elimination Method
                        </CardDescription>
                    </div>
                    <Popover>
                ...
                <CardFooter className="flex justify-between">
                    ...
                    <div className="flex items-center">
                        ...
                        <Button onClick={gaussEli}>Calculate</Button>
                    </div>
                </CardFooter>
            </Card>
            ...
            {result.length > 0 ? (
                <>
                    ...
                    <Card className='w-full md:w-[705px] xl:w-[1214px]'>
                        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                            <CardTitle>Solution by Gauss Elimination Method</CardTitle>
                        </CardHeader>
                        <CardContent>
                            {sol.map((row, i) => (
                            ...
                            {array.toReversed().map((row, i) => (
                                <div key={i} className="flex flex-row gap-4">
                                        <div className="">{`x${i+1} = `}</div>
                                        <div className="">{`${removeZero(row.sum)} / ${removeZero(row.pivot)} = ${removeZero(row.x)}`}</div>
                                </div>
                            ))}
                        </CardContent>
                    </Card>
