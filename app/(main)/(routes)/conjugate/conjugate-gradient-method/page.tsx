...
const ConjugateGradientMethod = () => {
    ...
    const conjugateGradient = () => {
        ...
        let k = 0;
        let r: number[] = math.subtract(math.multiply(matrixA, x), vectorB) as number[];
        let d: number[] = math.multiply(r, -1) as number[];
        let err: number  = 1;
        array.push({iter: k, x: math.clone(x), error: err});
        while (err >= error && k < maxIteration) {
            k++;
            const dtr = math.dot(d, r);
            const dtad = math.dot(math.multiply(math.transpose(d), matrixA), d);
            const lmda = -(dtr / dtad);
        
            const x1 = math.add(x, math.multiply(d, lmda)) as number[];
            let r1 = math.subtract(math.multiply(matrixA, x1), vectorB) as number[];
        
            // Define zero
            r1 = r1.map(val => (Math.abs(val) < error ? 0 : val));
        
            err = math.sqrt(math.dot(math.transpose(r1), r1)) as number;

            const r1tad = math.dot(math.multiply(math.transpose(r1), matrixA), d);
            const alpha = r1tad / dtad;
            const d1 = math.subtract(math.multiply(d, alpha), r1) as number[];
        
            console.log(`Iteration ${k}: ${x1} Error: ${err}`);
            
            array.push({ iter: k, x: math.clone(x1), error: err });
            d = d1;
            r = r1;
            x = x1;
          }
            ...
            <Card className="w-full md:w-[705px] xl:w-[1214px]">
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                    <div className='space-y-2'>
                        <CardTitle>Conjugate Gradient Method</CardTitle>
                        <CardDescription>
                            Find the solution of a system of linear equations using Conjugate Gradient Method
                        </CardDescription>
                    </div>
                    <Popover>
                ...
                <CardFooter className="flex justify-between">
                    ...
                    <div className="flex items-center">
                        ...
                        <Button onClick={conjugateGradient}>Calculate</Button>
                    </div>
                </CardFooter>
            </Card>
