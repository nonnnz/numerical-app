...
const OnePointMethod = () => {
    ...
    const onePointMethod = () => {
        ...
        while(ea > error && iter < maxIteration) {
            ...
            if(x != 0) ea = Math.abs((x-xold)/x)*100;
            else ea=0;

            array.push({
                iter: iter,
                x: x.toFixed(calculateDecimalPlaces(error)),
        ...
        <div className="h-full flex-1 flex-col  items-center space-y-4 p-8 md:flex">
            <Card className="w-full md:w-[705px] xl:w-[1214px]">
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                    <div className='space-y-2'>
                        <CardTitle>One-Point Method</CardTitle>
                        <CardDescription>
                            Find the root of the equation using One-Point Method <br></br> by rearranging the function f(x) = 0 so that x is on the left-hand side of the equation
                        </CardDescription>
                    </div>
                    <Popover>
                ...
                <CardFooter className="flex justify-between">
                    ...
                    <div className="flex items-center">
                        ...
                        <Button onClick={onePointMethod}>Calculate</Button>
                    </div>
                </CardFooter>
            </Card>
            ...
            {result !== null && (
                <>
                    ...
                    <div className="rounded-md border bg-card w-full md:w-[705px] xl:w-[1214px]">
                        ...
                        <TableHeader>
                            <TableRow>
                                ...
                                <TableHead className="">xi</TableHead>
                    ...
                    <div className="rounded-md border bg-card w-full md:w-[705px] xl:w-[1214px]">
                      <MyPlotComponent func={functionExpression} x={plotData.map((item) => item.x)} y={plotData.map((item) => item.y)} />
                    </div>
                </>
            )}
        </div>
