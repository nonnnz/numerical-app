...
const SecantMethod = () => {
    ...
    const secantMethod = () => {
        ...
        let array = [];
        let _x0 = parseFloat(x0);
        let _x1 = parseFloat(x1);
        let x2 = null;
        let iter = 0;
        let ea = 1;

        while (ea > error && iter < maxIteration) {
            x2 = _x1-((f(_x1)*(_x0-_x1))/(f(_x0)-f(_x1)));
            iter++;
            if(x2 != 0) ea = Math.abs((x2 - _x1) / x2) * 100;
            else ea = 0;

            array.push({
                iter: iter,
                x0: _x0.toFixed(calculateDecimalPlaces(error)),
                fx0: f(_x0).toFixed(calculateDecimalPlaces(error)),
                x1: _x1.toFixed(calculateDecimalPlaces(error)),
                x2: x2.toFixed(calculateDecimalPlaces(error)),
                error: ea.toFixed(calculateDecimalPlaces(error)),
                fx2: f(x2).toFixed(calculateDecimalPlaces(error))
            });
        ...
        <div className="h-full flex-1 flex-col  items-center space-y-4 p-8 md:flex">
            <Card className="w-full md:w-[705px] xl:w-[1214px]">
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                    <div className='space-y-2'>
                        <CardTitle>Secant Method</CardTitle>
                        <CardDescription>
                            Find the root of the equation using Secant Method
                        </CardDescription>
                    </div>
                    <Popover>
                ...
                <CardFooter className="flex justify-between">
                    ...
                    <div className="flex items-center">
                        ...
                        <Button onClick={secantMethod}>Calculate</Button>
                    </div>
                </CardFooter>
            </Card>
            ...
            {result !== null && (
                <>
                    ...
                    <div className="rounded-md border bg-card w-full md:w-[705px] xl:w-[1214px]">
                        ...
                        <TableHeader>
                            <TableRow>
                                ...
                                <TableHead className="">x0</TableHead>
                                <TableHead className="">x1</TableHead>
                                <TableHead className="">x2</TableHead>
                    ...
                    <div className="rounded-md border bg-card w-full md:w-[705px] xl:w-[1214px]">
                      <MyPlotComponent func={functionExpression} x={array.map((item) => item.x2)} y={array.map((item) => item.fx2)} />
                    </div>
                </>
            )}
        </div>
